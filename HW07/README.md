...
-   **checkpoint_timeout**  — максимальное время между контрольными точками. Если значение задано без единиц измерения, оно считается заданным в секундах. По умолчанию — 5 минут.
...
tps = 1792.035928
```
разница существенна, т.к. в асинхронном режиме:

- фиксация изменений не ждет подтверждения записи(т.е. медленного ввода\вывода на диск)
- сервер сообщает об успешном завершении сразу, как только транзакция будет завершена логически, не дожидаясь записи wal на диск.
